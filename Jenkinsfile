...
node {
    ...
    docker.image('jhipster/jhipster:v5.7.2').inside('-u jhipster -w /home/jhipster/app/ -e MAVEN_OPTS="-Duser.home=./"') {
        stage('check java') {
            sh "java -version"
        }
    ...
    stage('build docker') {
        ...
        dockerImage = docker.build('pascalgrimaud/jhipster190120', 'target/docker')
    }
}
